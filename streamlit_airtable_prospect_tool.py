    ...
    for src in selected_sources:
        try:
            ...
            try:
                ...
                logging.info(f"✓ Fetched {src['label']} with specific fields (faster)")
            except Exception as e1:
                # If that fails, fetch all fields (slower but more reliable)
                logging.warning(f"⚠ Could not fetch specific fields for {src['label']}, fetching all fields: {e1}")
                records = table.all()
            ...
            for r in records:
                ...
                # Get Domain field (try common variations) - optimized order
                domain_value = None
                
                # First, try exact field name matches (case-sensitive) - most common first
                ...
                # If no exact match, try URL fields and extract domain
                if not domain_value:
                    ...
                    for k, v in fields.items():
                        if isinstance(v, str) and v.strip():
                            ...
                            if "http" in v.lower():
                                ...
                                break
                            # Check if it looks like a domain (has dots and reasonable length)
                ...
                if d:
                    ...
                    if is_disavow_list:
                        ...
                        # For disavow lists, don't count as SAFE (they're always excluded)
                        # safe_count stays 0 for disavow lists
                    else:
            ...
            if show_progress:
                ...
                field_info = f" (using field: {domain_field_found})" if domain_field_found else ""
                logging.info(f"✓ Fetched {count:,} domains from {src['label']} ({src['base_id']}) - {active:,} active, {safe_count:,} SAFE{field_info}")
            
            if count == 0:
                logging.warning(f"⚠ {src['label']} returned 0 domains - check if table has records and field names")
                if fields_logged and 'fields' in locals():
                    try:
                        logging.warning(f"   Available fields were: {list(fields.keys())}")
                    except:
                        pass
...
with st.expander("📋 View detailed breakdown by source"):
    for src in active_sources:
        ...
        if is_disavow:
            ...
            active = count - safe
            st.write(f"  • **{src['label']}**: {count:,} total ({active:,} active, {safe:,} SAFE)")
