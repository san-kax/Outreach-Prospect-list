...
# Helper function to test base access
...
with st.expander("📋 View detailed breakdown by source"):
    ...
    if permission_issues:
        ...
        with st.expander("🔧 Troubleshooting Steps"):
            ...
            st.write("1. **Verify token match:** Ensure the token in Streamlit secrets matches the one you're editing")
            st.write("2. **Save token again:** In Airtable, click 'Save changes' on the token (even if already saved)")
            st.write("3. **Check scopes:** Ensure token has `data.records:read` scope enabled")
            st.write("4. **Verify base/table IDs:** Check that base_id and table_id in code match the actual Airtable base")
            st.write("5. **Wait a few seconds:** Token changes may take a moment to propagate")
