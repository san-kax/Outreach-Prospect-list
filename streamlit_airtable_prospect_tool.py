...
SOURCES = [
    ...
    {"label": "GDC-Disavow-List",    "base_id": "appJTJQwjHRaAyLkw", "table_id": "tbliCOQZY9RICLsLP"},
    {"label": "GDC-Disavow-List-1",  "base_id": "appEEpV8mgLcBMQLE", "table_id": "tbliCOQZY9RICLsLP"},
]
...
def is_domain_safe_to_reuse(record_fields: dict, months_threshold: int = 12) -> bool:
    ...
    date_field = record_fields.get("Date") or record_fields.get("date") or record_fields.get("Added Date")
    if not date_field:
...
def fetch_existing_domains(selected_sources: list[dict], show_progress: bool = False, exclude_old_domains: bool = True, months_threshold: int = 12) -> tuple[set[str], dict[str, int], dict[str, int]]:
    ...
    for src in selected_sources:
        try:
            ...
            # Fetch both Domain and Date fields
            records = table.all(fields=["Domain", "Date", "date", "Added Date"])
            count = 0
            safe_count = 0
            
            for r in records:
                fields = r.get("fields", {})
                d = normalize_domain(fields.get("Domain", ""))
                if d:
                    count += 1
...
st.caption("Domains older than 12 months are marked as SAFE and can be reused (excluded from duplicate check).")
