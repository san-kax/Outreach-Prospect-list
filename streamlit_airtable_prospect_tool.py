...
def fetch_single_source(src: dict, exclude_old_domains: bool, months_threshold: int) -> tuple[str, set[str], int, int, str | None]:
    ...
    try:
        ...
        try:
            ...
            if "403" in str(e1) or "Forbidden" in str(e1) or "INVALID_PERMISSIONS" in str(e1):
                logging.error(f"⚠ Permission denied for {src['label']} - Airtable token may not have access to this base")
            ...
            try:
                ...
                if "403" in str(e2) or "Forbidden" in str(e2) or "INVALID_PERMISSIONS" in str(e2):
                    logging.error(f"⚠ Permission denied for {src['label']} - Airtable token may not have access to this base")
                raise
        ...
        if "403" in error_str or "Forbidden" in error_str or "INVALID_PERMISSIONS" in error_str:
            logging.error(f"✗ Permission denied for {src['label']} ({src['base_id']})")
            logging.error(f"   Your Airtable token does not have access to this base.")
            logging.error(f"   Please grant read access to base: {src['base_id']} in Airtable.")
        else:
...
with st.expander("📋 View detailed breakdown by source"):
    for src in active_sources:
        ...
        if is_disavow:
            ...
            if count == 0:
                status_msg += " ⚠️ (Check Airtable permissions if this should have data)"
